...
export default function ProductEditPage() {
  ...
  // Manage product data, error message, and loading state
  ...
  useEffect(() => {
    const getProduct = async () => {
      try {
        ...
        if (!res.ok) {
          setErrorMessage(data.error || 'Failed to load product information.');
    ...
    // Fetch product data for current ID
    if (productId) {
      getProduct();
    }
  }, [productId]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Input validation (image file optional)
    if (!name.trim()) {
    ...
    try { // Send PUT request to product edit API
      ...
      if (res.ok) { // Navigate to admin product list on successful update
        router.push('/admin/products?edited=1'); // Notify success via query parameter
      } else {
        const data = await res.json();
