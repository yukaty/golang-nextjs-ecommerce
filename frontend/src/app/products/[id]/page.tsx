...
import { notFound } from 'next/navigation'; // For displaying 404 page
...
async function getProduct(id: string): Promise<Product | null> {
  try {
    ...
    // Convert data returned from API to JavaScript object
...
async function getReviews(id: string): Promise<ReviewsResponse | []> {
  // Get review list from reviews API
...
// Determine star display for review
function displayStars(avgRating: number) {
  ...
  const emptyStars = '☆'.repeat(5 - rating); // Remaining empty stars
  return `${filledStars}${emptyStars}`;
}

// Product detail page
export default async function ProductDetailPage(props: ProductDetailPageProps) {
  const resolvedParams = await props.params; // Await required because obtained asynchronously
  ...
  // Display 404 page if product not found
  if (!product) {
  ...
  // Handle case when stock doesn't exist
  ...
  // Display dummy image if no image specified
